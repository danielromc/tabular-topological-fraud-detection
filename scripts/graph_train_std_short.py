"""
Quick GraphSAGE training (standardization enabled) for comparison.
Saves metrics in metrics_graphsage_val_std_short.json
"""
import json
import time
...
train_mask_init = data["claim"].train_mask
mean = x_claim[train_mask_init].mean(dim=0, keepdim=True)
std = x_claim[train_mask_init].std(dim=0, keepdim=True).clamp(min=1e-6)
data["claim"].x = (x_claim - mean) / std
print('Using standardized features (train mean/std)')
...
metrics = {"model": "graphsage_std_short", "best_epoch": best_epoch, "training_time_sec": 0, "val": {"auc_pr": float(val_eval["auc_pr"]), "auc_roc": float(val_eval["auc_roc"])}}
with open(ART_GRAPH / "metrics_graphsage_val_std_short.json", "w") as f:
    json.dump(metrics, f, indent=2)
print('Saved metrics_graphsage_val_std_short.json')
